...
def load_model(symbol):
    ...
    if not model_path.exists():
        return None, f"Model file not found for {symbol}"
...
def backtest(symbol, forecast_horizon=1):
    ...
    model, model_warning = load_model(symbol)
    if model is None:
        return {"warning": model_warning}, None
...
if __name__ == "__main__":
    ...
    report_data["overall_accuracy"] = float(np.mean(accuracies)) if accuracies else None
