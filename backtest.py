...
try:
    ...
    if __name__ == "__main__":
        ...
        print(f"DATA_DIR exists: {DATA_DIR.exists()}, contents: {list(DATA_DIR.glob('*'))}")
        print(f"MODEL_DIR exists: {MODEL_DIR.exists()}, contents: {list(MODEL_DIR.glob('*'))}")
        print(f"REPORTS_DIR will be: {REPORTS_DIR}")
            
except Exception as e:
    print("ERROR in backtest.py:", e)
    traceback.print_exc()
    sys.exit(1)